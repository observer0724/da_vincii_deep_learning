...
file_name = '/home/yida/Desktop/training_pad_large.pkl'
...
num_valid = int(round(num_images*0.2))
...
def accuracy_test(predictions,test_label):
    accu = 100.0 * np.sum(np.argmax(predictions, 1) == np.argmax(test_label, 1))/ predictions.shape[0]
    result = np.argmax(predictions,1) == np.argmax(test_label,1)
    error_index_tuple = np.where(result == False)
    error_index = error_index_tuple[0]
    error_labels = []
    for i in range(np.shape(error_index)[0]):
        error_labels.append(np.argmax(test_label,1)[error_index[i]])
    return accu, error_labels
...
depth2 = 12 #the depth of 2nd convnet
C5_units = 120
F6_units = 84
F7_units = 2
...
with graph.as_default():
    ...
    with tf.name_scope('fc1'):
        C5_weights = tf.Variable(tf.truncated_normal([ (16-patch_size)* (16-patch_size)* depth2/4, C5_units], stddev=0.1), name = 'C5_weights')
    ...
    with tf.name_scope('loss'):
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf_train_labels, logits = logits), name = 'loss')
        # tf.scalar_summary('loss', loss)

    # Optimizer.
    with tf.name_scope('optimizer'):
        optimizer = tf.train.FtrlOptimizer(0.0005).minimize(loss)
...
# writer = tf.train.SummaryWriter('/home/yida/Desktop/paper/board/',sess.graph)
with sess as session:
    ...
    for step in range(num_steps):
        ...
        _, l, predictions= session.run(
          [optimizer, loss, train_prediction], feed_dict=feed_dict,options=None, run_metadata=None)
        ...
        if (step % 100 == 0):
          ...
          print('--------------------------------------')
    # saver.restore(sess, "/home/yida/Desktop/da_vincii/31*31_images/4*4with1and3_tf.train.GradientDescentOptimizer/damodel.ckpt")
    # print('Test accuracy: %.1f%%' % accuracy(test_prediction.eval(), test_labels))
    test_accuracy, error_label = accuracy_test(test_prediction.eval(), test_labels)
    print ("Test accuracy: %.1f%%" % test_accuracy)
    print ("wrongs labels are:")
    print (error_label)
    end_time = time.time()
    duration = (end_time - start_time)/60
    print("Excution time: %0.2fmin" % duration)
    save_path = saver.save(session, "/home/yida/Desktop/paper/damodel.ckpt")
